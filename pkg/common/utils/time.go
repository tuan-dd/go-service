...
var TimeZoneJapan *time.Location

func init() {
	TimeZoneJapan, _ = time.LoadLocation("Asia/Tokyo")
}
...
func StringToTime(dateStr, frm string) (*time.Time, *response.AppError) {
	...
	if err != nil {
		return nil, response.NewAppError("invalid time format", constants.TimeInvalid)
	}

	return &parsedTime, nil
}

func StringToTimeZone(dateStr, frm string, tz *time.Location) (*time.Time, *response.AppError) {
	if tz == nil {
		tz = TimeZoneJapan
	}

	parsedTime, err := time.ParseInLocation(string(frm), dateStr, tz)
	if err != nil {
		return nil, response.NewAppError("invalid time format", constants.TimeInvalid)
	}
